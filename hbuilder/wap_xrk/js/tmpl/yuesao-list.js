$(function() {
	...
	//获取分类
	function getMenu() {
		console.log($classInfo.gcId);
		$.ajax({
			type: "get",
			url: ApiUrl + "/index.php?act=service_goods_class&op=get_peer_class",
			data: {
			...
			success: function(data) {
				...
				if(data.code == 200) {
					for(var i = 0; i < data.datas.length; i++) {
						if(data.datas[i].gc_name == '月嫂' || data.datas[i].gc_name == '育婴师') {
							$nav += '<a data-id="' + data.datas[i].gc_id + '">' + data.datas[i].gc_name + '</a>';
						}
					}

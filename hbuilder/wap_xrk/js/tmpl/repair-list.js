...
//					html = '<img src="../images/banner_02.jpg" alt="">';
	...
	function getCityArea() {
		$.ajax({
			...
			success: function(data) {
				_.debug(data);
				console.log("城市返回数据" + JSON.stringify(data));
				var _area = "";
	...
	function getListData(curpage, pagesize, orderKind, order, pullDirect) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				console.log("getlist");
				console.log("列表返回数据"+JSON.stringify(data));
	...
	function get_kind_list() {
		$.getJSON(ApiUrl + '/index.php?act=service_class&op=get_child_all', {
			...
			var str = template.render("repairTemp",result);
			$(".repair").html(str);
			
			$(".repair .repair-list").on("tap",function(){
				var $this = $(this);
				$this.addClass("active").siblings().removeClass("active");
				$classInfo.gcId = $this.attr('data-gc-id');
				console.log('classid==='+$classInfo.gcId);
				pulldownRefresh();
			})
		})
	}

...
//	var $classInfo = JSON.parse(decodeURIComponent(getQueryString('classInfo'))) ;
//	console.log($classInfo);
	var $cityInfo = _.getCityInfo() || _.getDefaultCityInfo;
	$cityInfo.area_id = '224';
	//跑腿分类默认为2
	var gc_id = $('#runner_daisong .runbuy-buttons').find('.active').attr('gc_id');
	console.log(gc_id);
	var typeid = 37;
	...
	function getDetail(typeid){
		$.ajax({
			...
			dataType: "json",
			success: function(res){
				$('#tips_detail .mui-tips').html(res.datas.type_info.books_desc);
			}
		});
	}
	...
	$('.runbuy-protocol a').on('tap',function(){
		mui.openWindow({
			url: '../common/type_agreement.html?type_id=' + typeid,
			id: "type_agreement"
		})
	})
	$('.runbuy-Price a').on('tap',function(){
		mui.openWindow({
			url: '../common/type_price.html?type_id=' + typeid,
			id: "type_price"
		})
	})
	//初始化控件
...
//		calcPrice();
	...
	function submit_daisong() {
		...
		$.sValid.init({
			...
			messages: {
				...
				address_from_daisong: "请输入取件地址！",
				true_name_to_daisong: "请输入送件联系人！",
				mob_phone_to_daisong: {
					"required": "请输入收件人手机！",
					"mobile": "收件人手机格式不正确！"
				},
				...
				needs_time_from_daisong: "请输入取件时间！",
		...
		if($.sValid()) {
			...
			for(var i = 0; i < _form_param.length; i++) {
				...
				if(field_name == 'needs_amount'){
					text  = value + '元';
				}
				if(field_name == 'needs_fee'){
					text  = value + '元';
				}
				if(field_name == 'needs_price'){
					text  = value + '元';
				}
				if(field_name == 'needs_distance'){
					text  = value + 'km';
				}
			...
			mui.openWindow({
				url: '../order/service_buy.html?param='+JSON.stringify(data),
				id: 'service_buy'
			})
		}
	}
	...
	//寄件地址选择   判断fill_address_from_daisong
	var eventData_from = window.localStorage.getItem('fill_address_from_daisong');
	if(eventData_from){
		var $param = JSON.parse(decodeURIComponent(eventData_from) );
		...
		$("#address_from_daisong_txt").text($param.address + $param.detail);
	...
	//收件地址选择    判断fill_address_to_daisong
	var eventData_to = window.localStorage.getItem('fill_address_to_daisong');
	if(eventData_to){
		var $param = JSON.parse(decodeURIComponent(eventData_to) );
		...
		$("#address_to_daisong_txt").text($param.address + $param.detail);
	...
	$('.runbuy-address-item').on('tap', function() {
		...
		mui.openWindow({
			url: "address_select.html?eventName="+eventName,
			id: "address_select"
		})
	})
	$('.runbuy-address-item.mai').off('tap').on('tap', function() {
		...
		var eventName = $this.attr('eventName');
		mui.openWindow({
			url: "publish_needs_select_goods.html?eventName="+eventName,
	...
	function calDistance() {
		...
		if(type_model == 1) {
			...
			if(lng_from && lat_from) {
				var lnglat = new AMap.LngLat(lng_from, lat_from);
			}
			//取得代送的到坐标
			var lng_to = $("#address_to_daisong").attr('lng');
			var lat_to = $("#address_to_daisong").attr('lat');
			if(lng_to && lat_to) {
				distance = Math.ceil(lnglat.distance([lng_to, lat_to])/1000);
			...
			if(lng_to && lat_to) {
				distance = lnglat.distance([lng_to, lat_to]);

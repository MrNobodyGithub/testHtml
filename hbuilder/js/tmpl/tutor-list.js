$(function(){
	/**
	 * @author:zhaoyangyue 
	 * @description 根据common-list.js修改上拉加载下拉刷新 及 列表数据请求
	 * @version: 2017-7-21
	 */
	mui.init({
		swipeBack: true,
	...
	mui.plusReady(function() {
		...
		$(".comkind .mui-collapse-content").on("click", "i", function() {
			console.log($(this).text());
			$(".nanny").show().siblings("section").hide();
			var gc_id = $(this).attr('data-gc-id');
			console.log(gc_id);
			$classInfo.gcId = gc_id;
			loadPage();
		})
//		loadPage();
		/*
		//以下方法可以调用webview的原生下拉刷新
		ws.setPullToRefresh({
			support: true,
			height: "50px",
			range: "200px",
...
//		mui('.content').pullRefresh().pulldownLoading();
	...
	document.addEventListener('changecity', function(event) {
		...
		console.log('change city:' + JSON.stringify($cityInfo));
	...
	$(".screening .confirm").on('tap', function() {
		//设置高级搜索条件
		var minPrice = $('.price .start').val();
		var maxPrice = $('.price .end').val();
		filterCondition = {
...
//		});
	...
	function get_kind_list() {
		$.getJSON(ApiUrl + '/index.php?act=service_class&op=get_child_all', {
			sc_id: $classInfo.gcId,
			area_id: $cityInfo.area_id,
			globalLoading: true
		}, function(result) {
			...
			$(".comkind .text").html($classInfo.className);
			
			if($classInfo.gcId == 9){
				$(".comkind .icon").css({
					"background": "url(../images/kind/kind_02.jpg) no-repeat center center",
					"background-size": "0.8rem 1rem"
				})
			}else if($classInfo.gcId == 26){
				$(".comkind .icon").css({
					"background": "url(../images/kind/kind_09.png) no-repeat center center",
					"background-size": "1rem 1rem"
				})
			}
			
			$(".comkind .mui-collapse-content").on("click", "i", function() {
				console.log("关键字："+$(this).text()+"gc_id:"+$(this).gc_id);
				$(".nanny").show(300).siblings("section").hide(300);
				var gc_id = $(this).attr('data-gc-id');
				$classInfo.gcId = gc_id;
				loadPage();
				
				//点击返回分类页
				$('.nanny .icon-back').on('tap', function(){
					back();
				})
			})
		})

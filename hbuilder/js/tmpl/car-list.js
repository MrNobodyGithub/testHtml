$(function() {
	...
	function getMenu(){
		$.ajax({
			type: "get",
			url: ApiUrl + "/index.php?act=service_class&op=get_child_all",
			data: {
			...
			success: function(data) {
				var $nav = '';
				if(data.code == 200) {
					for(var i = 0; i < data.datas.class_list.length; i++) {
						$nav += '<a data-id="' + data.datas.class_list[i].gc_id + '">' + data.datas.class_list[i].gc_name + '</a>'
					}
					$('.filter1 nav').append($nav).find('a').eq(0).addClass('active1');
					$classInfo.gcId = $('.filter1 nav').find(".active1").attr("data-id");
					//加载数据
					loadPage();
					
					//菜单切换事件
					$('.filter1 nav').on('tap', 'a', function() {
						var $this = $(this);
						var $index = $this.index();
 						if(!$this.hasClass("active1")) {
 							$('.filter1 nav').find('a').removeClass('active1').eq($index).addClass('active1');
							$classInfo.gcId = $('.filter1 nav').find(".active1").attr("data-id");
							$('.filter nav').find('a').removeClass('active').removeClass('changeBg').eq(0).addClass('active');
							$('.product').empty();
							loadPage();
						}
					});

			...
			mui.plusReady(function(){
				...
				console.log(self.addressId)
				//各种类型页面切换  
				/*$(".address_select").on("tap",function(){
				    mui.openWindow({
					    url:"run-address.html",
				...
				$(".add-li").on("tap",function(){
				    mui.openWindow({
					    url:"runerrands-map.html",
					    createNew:false,//是否重复创建同样id的webview，默认为false:不重复创建，直接显
					    waiting:{
					      autoShow:false,//自动显示等待框，默认为true
					      title:'正在加载...'//等待对话框上显示的提示内
				...
				$.ajax({
					...
					success:function(res){
						...
						$(".addman").val(data.datas.address_info.true_name)
						$(".addtel").val(data.datas.address_info.mob_phone)
						$(".cityadd").val(data.datas.address_info.area_info)
						$(".detailadd").val(data.datas.address_info.address)
					}
				});

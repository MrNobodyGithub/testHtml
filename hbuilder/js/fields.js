	...
	htmlField : function($config) {
		...
		switch($fieldname) {
			...
			case 'address_from':
				...
				$html += '<input data-label="'+$config['field_title'] + '" type="text" id="address_from" name = "address_from" value="' + $value + '" placeholder="' + $config['field_tips'] + '" readonly>';
			...
			default:
				...
				//
				console.log(JSON.stringify(that.validparams));
	...
	initcontrols : function() {
		...
		$('.addresssearch').each(function(){
			...
			$this.on('tap', function() {
				mui.openWindow({
					url: "/tmpl/common/address_select_map.html",
		...
		$('.sun_address_select').each(function(){
			...
			window.addEventListener(eventName, function(event) {
				var $param = event.detail;
				//console.log(JSON.stringify($param));
				$input.val(($param.address || '') + '('+($param.title || '')+')');
				$input.attr('city',$param.city || '');
				$input.attr('district',$param.district || '');

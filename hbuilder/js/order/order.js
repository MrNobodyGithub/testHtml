...
//	console.log("totalPrice-----" + totalPrice);
	...
	if(is_face == 1) {
		 ...
		 * @version: 2017-7-21
		 */
		/*$('#page-coupon-voucher').hide();
		$('#page-coupon-card').hide();
		$('#coupon-btn-voucher').hide();
		$('#coupon-btn-card').hide();*/
		...
		if(paymun<1){
			...
			_.data.send(create_url, 'post', true, data, function(data) {
				if(data.code == 200) {
					...
					console.log("-----------" + datas.order_info.final_pay_amount);
	...
	$('.btn-yellow').on('tap', function() {
		...
		if(is_face == 0) {
			...
			_.data.send(create_url, 'post', true, data, function(data) {
				if(data.code == 200) {
					console.log("submitnum==="+submitmun);
					...
					/*mui.openWindow({
					    ...
					    extras:{order_info:order_info,is_face:is_face}
					})*/
...
$(function() {
	console.log("self==="+JSON.stringify(param));
	//支付密码
	//  $("#card_btn").on('tap',function(){
	//  	var passUrl=ApiUrl+"/index.php?act=member_account&op=check_paypwd";
	//  	$.ajax({
	//  		type:"post",
	//  		url:passUrl,
	//  		async:true,
	//  		dataType:"json",
	//  		data:{
	//  			key:_.userInfo.getKey(),//"e637ed1888b5b9dd9df2599ade93c813",//key ,//"e637ed1888b5b9dd9df2599ade93c813",
	//  			password:$("#txtPassword").val()
	//  		},
	//  		success:function(data){
	//  			console.log(data);
	//  			//$("#layPassword").css("display","none");
	//  			if(data.datas.error!="支付密码输入不正确"){
	//  				$("#layPassword").css("display","none");
	//  			}
	//  		}
	//  	});

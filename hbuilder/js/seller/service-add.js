$(function() {
	...
	var curTypeid = 0;
	var is_gv = 0;//是否虚拟商品
	//var $key = '18473611494a29dab09a10fa101df880';
	bindEvent();
	get_kind_list();
	getAreaList();
	/**
	...
	function bindEvent() {
		...
		$('.comkind').on('click', '.mui-collapse-content i', function() {
			var
				...
				// 搬家和維修固定金額
				curTypeid = $typeId;
				console.log(curTypeid);
				if(curTypeid == 33){
					$("#g_price").val("200").attr("readonly", "readonly");
				}else if(curTypeid == 42){
					$("#g_price").val("50").attr("readonly", "readonly");
				}
			// 传值
		...
		$("footer a").on("click", function() {
			...
			if($.sValid()) {
				...
				//检查上传的图片是否超过10张
				if(picCount > 10){
					mui.toast("您上传的图片超过了10张,请删除多余的图片");
					return false;
				}
				...
				//console.log(parseParam(_form_param));
				//提交服务
				$.ajax({
					type: "post",
					url: ApiUrl + "/index.php?act=seller_service&op=service_add",
					async: true,
	 ...
	 * 获取规格列表
	 * @param {Object} $gcId
	 * index.php?act=service_class&op=get_spec此接口废弃
	 * 新接口地址：index.php?act=seller_service&op=get_spec
	 */
			...
			success: function(data) {
				...
				if(data.code == 200) {
					...
					if(data.datas.spec == null) {
						...
						var specListTpl = template.render('specListTpl', {speclist:data.datas.spec});
						...
						$('dl[type="spec_group_name"]').on('click', 'ul li input[type="checkbox"]', function() {
							...
							console.log(curTypeid);
							if(curTypeid == 33){
								$("#specPrice input").val("200").attr("readonly", "readonly");
							}else if(curTypeid == 42){
								$("#specPrice input").val("50").attr("readonly", "readonly");
							}
						});
					}
					$("#skuList").html('');
					...
					if(data.datas.attr == null) {
						...
						var attrListTpl = template.render('attrListTpl', {attrlist:data.datas.attr});
